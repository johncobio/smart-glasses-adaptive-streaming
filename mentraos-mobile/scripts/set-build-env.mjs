...
export async function setBuildEnv() {
  let gitCommit = "nogit", gitBranch = "nogit", gitUsername = "nogit"
  try {
    gitCommit = (await $`git rev-parse --short HEAD`).stdout.trim()
    gitBranch = (await $`git rev-parse --abbrev-ref HEAD`).stdout.trim()
    gitUsername = (await $`git config user.name`).stdout.trim().replace(/ /g, "_")
  } catch (_) {
    // not a git repo — use placeholder values
  }
